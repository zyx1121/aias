...
When a model does not fit, `model_up` changes nothing and returns `{refused, fits, need_mib, free_mib, evict}`. `evict` is the fewest models to stop (among sets of that size, the one idle the longest), never pinned ones or ones running a job; pass `evict: "auto"` to stop exactly those. The job plans again once it holds the placement lock and stops the whole set or nothing. If even that is not enough, `evict` is empty and the reason says so. vLLM and nemo hold one model each, so asking for a different one replaces it (unless it is pinned or busy).
